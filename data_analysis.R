...
mean_cpu_time_exams = function(data_set, code_type, transform_var){
  ...
  for (prob in prob_set) {
    ...
    if (transform_var) {
      ...
      plot(sqrt(exams_set), mean_cpu_time, xlab = "Number of Exams", ylab = "Mean CPU Time",
           main = sprintf("Probability of colision %.2f", prob), pch = 19)
      regress = lm(mean_cpu_time ~ sqrt(exams_set))
    }
...
mean_cpu_time_probs = function(data_set, code_type, transform_var){
  ...
  for (exam in exams_set) {
    ...
    if (transform_var) {
      mean_cpu_time = sqrt(mean_cpu_time)
      plot(sqrt(prob_set), mean_cpu_time, xlab = "Probability of Collision", ylab = "Mean CPU Time",
           main = sprintf("Number of Exams %i", exam), pch = 19)
      regress = lm(mean_cpu_time ~ sqrt(prob_set))
    }
...
regressions = mean_cpu_time_plot(outputdata, code_type,"Exams", transform_vars = TRUE)
